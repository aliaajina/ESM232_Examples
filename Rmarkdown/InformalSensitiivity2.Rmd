...
tmp = map_df(site2,`[`, c("annual")) # Out of this list, extract annual
...
mean_elect = map_df(site2,`[`, c("mean")) # extracting all of the mean values here - we use the bracket to extract just mean
...
ggarrange(p1,p2) # uses ggarrange to save a plot to a variable - this puts the two plots together side by side
...
eff = rnorm(mean=0.6, sd = 0.1, n=nsamples) # sampling for a normal distribution

parms = cbind.data.frame(eff, ethresh) # combining them in a data frame where each column of the data frame is the parameters value

# use pmap 
# takes function name and then names of all parameters that don't change
results = parms %>% pmap(solarpv,  area=0.1, 
                         solar=sierraczosolar, clr="green",
                         eunit="W", g=FALSE, etype="direct") # map says i don't know which variable you want me to change so use my .x - pmap actually goes by the variable names in the data frame so it knows to use efficiency for eff and ethresh for ethresh - which means you have to name your columns
...
# and we can add the parameter values for each run as another column - will now have value of mean & ethreshold - since the annual value is an array it doesn't fit nicely (since it's year 1, y2, y3, etc)
...
# that the efficiency is more of a dependent variable than threshold radiation? threshold radiation doesn't seem to have any linear effect - the threshold parameter is the deviation in this line- it's must less important
# here we are coloring by the other parameter so you can see how the parameters are related
...
# using a for loop - for loops are really slow in R - pmap is must faster
# start by creating a data structures to hold results, separate 
# for scenario mean and annual values
# in for loops you have to set up the data structure to source the results - pmap automatically makes it for you - so here we are making the dataframe to store the results
...
#1944 is so low because it only had half the year of recorded values
```
...
#### don't have the hydro power function in our R folder
