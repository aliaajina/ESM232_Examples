...
site2$mean #comparing site 2 mean

```

Informal Sensitivity Analysis

**Informal** - try varying parameters
Decide which parameter(s) and what range to vary the parameter over - what is the uncertainty?
...
# Map runs this function for the list of random values - we tell it to do this for the different *eff* values; that we just created out of random dataset

head(site2) # now I have an annual array and mean value array
...
tmp = map_df(site2,`[`, c("annual"))  # can also use the map function to extract something from a list; can also use map function to repeat an extraction from a list over and over again - square bracket means extract! - this is telling it to take out all of the annual values from the list
...
#### Question: How does group=1 remove the grouping?
